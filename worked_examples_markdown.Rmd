...
The following three worked examples are intended to provide an easy step-by-step guide on how to infer dominance hierarchies, and their uncertainty, steepness and transitivity from dyadic interactions. These analyses can, however, be expanded depending on the researchers' interests. We imported the interaction datasets from a compilation of published studies carried out by Shizuka and McDonald (2015a,b). The three worked example accompany:
...
more information can be found at the Open Science Framework project: http://doi.org/10.17605/OSF.IO/9GYEK
...
First, we need to load the necessary libraries for the analysis and clear up the memory.
...
Then, we can upload the interaction matrix and visualize part of it to make sure it was imported in the right format.
...
This shows that the ids of the `r table(names(horse)==row.names(horse))` individuals are the same in both sides of the matrix. One further step is to make sure that all individuals interacted at least once. We can run the following code.
...
We can now count the number of interactions recorded and, thus, estimate the ratio of interactions to individuals (i.e. sampling effort).
...
This ratio of interactions to individuals (`r round(sum(horse)/(table(rowSums(horse)+colSums(horse)!=0)),1)`) shows an extraordinary sampling effort that is far beyond both the 10-20 recommendation and what is normally reported in the literature (reviewed by McDonald & Shizuka 2013).

Next step is to estimate how sparse the database is. For that, we will estimate the proportion of known dyads (i.e. the proportion of dyads that interacted out of the total number of possible dyads) and compare it to the proportion that we would expect when the probability of interacting follows a Poisson process, which is the process commonly observed in nature (more details in the main text).
...
Using the randomized Elo-rating the hierarchy looks like:
...
We can now quantify the uncertainty of the hierarchy, which also provides information about the steepness of the hierarchy. For that, we can use the function estimate_uncertainty_by_repeatability() from the package "aniDom" (Farine & S?nchez-T?jar 2017).

```{r}
# Uncertainy/steepness based on Elo-rating repeatability
...
Again, since this value is very high (mean = `r round(halve[[1]],2)`, 2.5 % and 97.5 % quantiles = (`r round(halve[[2]],2)`,`r round(halve[[3]],2)`)), the results corroborate that our estimated hierarchy is highly certain. 

Lastly, we can explore the transitivity of the interactions observed using Triangle transitivity (Ttri; Shizuka & McDonald 2012). Ttri has been shown to perform better than the widely used linearity index h' proposed by de Vries (1995), and it provides an overall estimate of orderliness for the estimated hierarchy.
...
This shows us that the ids of the `r table(names(monkey)==row.names(monkey))` individuals are the same in both sides of the matrix. One further step is to make sure that all individuals interacted at least once. We can run the following code.
...
Next step is to estimate how sparse the database is. For that, we will estimate the proportion of known dyads (i.e. the number of dyads that interacted out of the total number of possible dyads) and compare it to the proportion we would expect when the probability of interacting follows a Poisson process, which is commonly observed in nature (more details in the main text).
...
This value also shows that the hierarchy is not very steep, and our estimates not extremly certain, yet informative.

Lastly, we can explore the transitivity of the interactions observed using the Triangle transitivity (Ttri; Shizuka & McDonald 2012). Ttri has been shown to perform better than the widely used linearity index h' proposed by de Vries (1995), and it provides an overall estimate of orderliness for the hierarchy.
...
This shows us that the ids of the `r table(names(eagle)==row.names(eagle))` individuals are the same in both sides of the matrix. One further step is to make sure that all individuals interacted at least once. We can run the following code.
...
The shape observed indicates that this is a very flat (possibly non-existent) hierarchy, i.e. rank does not predict very well the probability of winning an interaction.

We can now quantify the uncertainty of the hierarchy, which also provides us with information about the steepness of the hierarchy. For that, we can use the function estimate_uncertainty_by_repeatability() from the package "aniDom" (Farine & S?nchez-T?jar 2017).
...
Overall, we conclude that this group of wintering white-tailed eagles shows a very flat (possibly non-existent) and intransitive hierarchy. Therefore, our estimates of rank order are extremely uncertain. Since the ratio of interactions to individuals recorded is within the recommended range, we can further conclude that this scenario is unlikely to be different after more data is collected.



### References

Farine, D.R. & S?nchez-T?jar, A. (2017) aniDom: Inferring Dominance Hierarchies and Estimating Uncertainty. URL: https://cran.r-project.org/package=aniDom 
